...
>
> 解决方法应该是：先对 $P(y|x)$ 取 log，然后再乘以一个归一化项 $\frac{1}{T^\alpha}$（$T$ 和长度正相关，长度越长，项越小）。使得越负的值，缩减效应更大，限制其负的程度。
...
TermFrequency_{t,d}=
\begin{cases}
tf_{t,d}=\operatorname{count}(t,d) \\
tf_{t,d}=\log_{10}(\operatorname{count}(t,d)+1)
\end{cases}
$$
...
$$
InverseDocumentFrequency(IDF)_t=\log_{10}\frac{N}{n_t}
$$
...
![](https://uricc.ga/images/2019/12/31/_20191231214446.png)

**算法:** 迭代

$$
\text{At } t=0:\quad PR(p_i,0)=\frac{1}{N}
$$

意味着一开始用平均值初始化每个 Page(i) 的 PageRank 权重。

$$
\text{At each time step:}\quad PR(P_i,t+1)=(1-d)+d\sum_{j\in In(P_i)}\frac{1}{|Out(P_j)|}PR(P_j,t)
$$

在每个新的 time step，某个 Page(i) 的新的权重等于【所有进入 Page(i) 的那些其他 Pages 的 $\frac{1}{|Out(P_j)|}$ 乘以它们自己的当前 PR 值】。
...
TextRank 与 PageRank 唯一的不同之处是多了 edge 之间的权重 $w$。
...
![](https://uricc.ga/images/2020/01/06/_20200106151459.png)
